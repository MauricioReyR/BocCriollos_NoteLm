...
>
  ...
  <div class="glass rounded-2xl overflow-hidden h-full flex flex-col transition-all duration-300 hover:border-warm-orange-500/80 hover:bg-warm-orange-500/5 hover:shadow-premium-lg">
    ...
    <div class="relative h-40 sm:h-48 overflow-hidden bg-gradient-to-br {{ $imageGradient ?? 'from-burnt-red-500 to-warm-orange-600' }}">
      ...
      {{-- Hover Shine Effect --}}
      <div class="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent translate-x-full group-hover:translate-x-0 transition-transform duration-500"></div>
    </div>
    ...
    <div class="flex-1 p-5 sm:p-6 flex flex-col justify-between">
      ...
      <div>
        ...
        <p class="text-sm text-gray-400 line-clamp-3 break-words group-hover:text-gray-300 transition-colors duration-300">
          {{ $description }}
        </p>
      </div>
      ...
      <div class="mt-4 pt-4 border-t border-white/10 flex items-center justify-between gap-4">
        ...
        {{-- CTA Button --}}
        <a 
          href="https://wa.me/573001234567?text=Quiero%20{{ urlencode($name) }}"
          target="_blank"
